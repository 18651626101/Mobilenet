...
__global__ void convGroupKernel(double *input,double *filter,double *output, double* bias, int filter_width,int filter_num,int input_width,int input_depth,int stride,int out_width, bool clip){
    ...
    for(int fnum=0;fnum<filter_num;fnum++){
        ...
        for(int r=0;r<filter_width;r++)
        for(int c=0;c<filter_width;c++)
            tmp += input[fnum*input_width*input_width + (input_row+r)*input_width + input_col+c]*filter[fnum*filter_width*filter_width + r*filter_width + c];
...
__global__ void convGroupKernel(double *input,double *filter,double *output, double* bias, int filter_width,int filter_num,int input_width,int input_depth,int stride,int out_width, bool clip){
    ...
    if(output_row >= out_width || output_col >= out_width) return;
        ...
        for(int r=0;r<filter_width;r++)
        for(int c=0;c<filter_width;c++){
            // if (output_row == 0 && output_col==0 && layer_idx==0)
            //     printf("%d, %d: %f * %f\n", r, c, input[layer_idx*input_width*input_width + (input_row+r)*input_width + input_col+c], filter[layer_idx*filter_width*filter_width + r*filter_width + c]);
...
double* conv(const int input_depth, const int input_width, 
    ...
    // cudaMalloc(&filter_cuda,sizeof(double)*filter_num*input_depth*filter_width*filter_width);
    ...
    // cudaMalloc(&bias_cuda,sizeof(double)*filter_num);
    // cudaMemcpy(filter_cuda, filter, sizeof(double)*filter_num*input_depth*filter_width*filter_width, cudaMemcpyHostToDevice);
    filter_cuda = filter;
    // cudaMemcpy(bias_cuda, bias, sizeof(double)*filter_num, cudaMemcpyHostToDevice);
    bias_cuda=bias;
    ...
    // printf("conv::get output%d\n",out_width);
    // cudaMemcpy(output, output_cuda, sizeof(double)*filter_num*out_width*out_width, cudaMemcpyDeviceToHost);
